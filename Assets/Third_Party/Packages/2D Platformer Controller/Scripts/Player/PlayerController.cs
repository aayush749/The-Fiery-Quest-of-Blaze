...
namespace SupanthaPaul
{
	public class PlayerController : MonoBehaviour
	{
		...
		{
			// horizontal input
			moveInput = inputAction.Gameplay.MoveRight.ReadValue<float>() - inputAction.Gameplay.MoveLeft.ReadValue<float>();
			...
			if (!isDashing && !m_hasDashedInAir && m_dashCooldown <= 0f)
			{
				...
				if (inputAction.Gameplay.Dash.ReadValue<float>() > 0.0f)
				{
					isDashing = true;
					// dash effect
			...
			// Jumping
			if(inputAction.Gameplay.Jump.ReadValue<float>() > 0.0f && m_extraJumps > 0 && !isGrounded && !m_wallGrabbing)	// extra jumping
			{
			...
			else if(inputAction.Gameplay.Jump.ReadValue<float>() > 0.0f && (isGrounded || m_groundedRemember > 0f))	// normal single jumping
			{
				m_rb.velocity = new Vector2(m_rb.velocity.x, jumpForce);
				// jumpEffect
				PoolManager.instance.ReuseObject(jumpEffect, groundCheck.position, Quaternion.identity);
			}
			else if(inputAction.Gameplay.Jump.ReadValue<float>() > 0.0f && m_wallGrabbing && moveInput!=m_onWallSide )		// wall jumping off the wall
			{
				m_wallGrabbing = false;
				m_wallJumping = true;
			...
			else if(inputAction.Gameplay.Jump.ReadValue<float>() > 0.0f && m_wallGrabbing && moveInput != 0 && (moveInput == m_onWallSide))      // wall climbing jump
			{
				m_wallGrabbing = false;
				m_wallJumping = true;
